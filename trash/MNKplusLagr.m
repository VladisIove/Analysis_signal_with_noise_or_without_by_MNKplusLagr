...
%Функція
%-----------------------------------------
f=200;%Частота періодичного сигналу
T=1/f;%Період сигналу
p=1;%Кількість періодів у сигналі
N=8192;%Кількість відліків (точок) у сигналі та Кількість точок для інтерполяції
...
xi=x;
        ...
        PohOtn=PohAbs./L*100;
m = min(PohOtn);
Info = ['p ' num2str(p) ' N ' num2str(N) ' f ' num2str(f) ' min ' num2str(m)];
disp(Info);
%{
